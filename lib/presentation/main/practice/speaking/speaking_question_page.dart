          ...
          body: Column(
            children: <Widget>[
              ...
              (widget.timeLimit.inSeconds > 0)
                ? TimeCounter(timeLimit: widget.timeLimit, navigateToNextPage: () {
                    Navigator.pushNamed(context, RoutesName.speakingResultRoute,
                      arguments: [isCorrectList, widget.part.index]);
                  })
                : Container(),           
              Expanded(
                child: Padding(
...
class _SpeakingQuestionPageBodyState extends State<SpeakingQuestionPageBody> {
  ...
  void initState() {
    ...
    for (int i = 0; i < widget.questionList.length; i++) {
      widget.isCorrectList.add({false: '...'});
    }
  }
  ...
  void _stopListening() async {
    ...
    setState(() {
      if (simplifyText(_lastWords) == simplifyText(_answer)) {
        ...
        widget.isCorrectList[_pageController.page!.round()] = {true: _lastWords};
      } 
      else 
      {
        widget.isCorrectList[_pageController.page!.round()] = {false: _lastWords};
        _lastWords = '';
            ...
            CommonButton(
              ...
              action: () {
                ...
                if (index == widget.questionList.length - 1) {
                  Navigator.pushNamed(context, RoutesName.speakingResultRoute,
                  arguments: [widget.isCorrectList, widget.partIndex]);
                } else {
